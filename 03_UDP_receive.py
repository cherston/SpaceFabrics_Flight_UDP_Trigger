...
parser.add_argument('-tl','--tl',type=float,default=2) #local save threshold
parser.add_argument('-tt','--tt',type=float,default=3) #transmit save threshold
...
parser.add_argument('-master','--master',nargs="+", type=int) # sets channel that should be treated as master in calibration
parser.add_argument('-drop','--drop',nargs="+", type=int) #drop channels
...
def savedata_slowcontrol(s_data):
	...
	# whenever saving a file, check storage and delete old files if needed
	remove_data_check()
